...
import com.mobius.ra.core.thread.GenerateReportRedoThread;
	...
	private static void generateReports(String args[], String operator, List<Report> reports) throws InterruptedException {
		...
		if (Constants.GENERATE_ORDER_PARALLELLY.equals(Constants.coreCfg.getGenerateOrder())) {
			for (Report report : reports) {
				if (report.getActive() != Constants.ACTIVE_DEFAULT || report.getRedoSwitch() != Constants.REDO_SWITCH_DEFAULT)
					continue;

				String reportName = report.getName();
				logger.info("Redo " + reportName + " report  is starting....******");

				GenerateReportRedoThread reportThread = new GenerateReportRedoThread(args, operator, report);
				reportThread.start();

				logger.info("Redo " + reportName + " report is running....******\n");
			}
		} else {// serially
			GenerateReportRedoThread reportThread[] = new GenerateReportRedoThread[reports.size()];
			for (int i = 0; i < reports.size(); i++) {
				if (reports.get(i).getActive() != Constants.ACTIVE_DEFAULT
						|| reports.get(i).getRedoSwitch() != Constants.REDO_SWITCH_DEFAULT)
					continue;

				String reportName = reports.get(i).getName();
				logger.info("Redo " + reportName + " report  is starting....******");

				reportThread[i] = new GenerateReportRedoThread(args, operator, reports.get(i));
				...
				logger.info("Redo " + reportName + " report is complete....******\n");
			}
		}
	}
}