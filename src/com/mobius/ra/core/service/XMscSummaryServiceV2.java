 ...
 * @version v 1.0
 */
public class XMscSummaryServiceV2 implements ServiceInterface {
	...
	public boolean summary(String args[], String operator, Report report) throws SQLException, ParseException {
		...
		while (c.compareTo(base) > 0) {
			...
			if (report.getMscFilenameUpdate() == Constants.INT_ONE) {
				fileNameHashMap = dao.getFileNameHashMap(yes,
						Constants.DB_NAME_MEDIATOR + Constants.coreCfg.getOperatorCode());
			}
			...
			for (int i = 0; i < billingThreadNum; i++) {
				...
				if (i < billingThreadNum - 1) {
					subList = billingList.subList(billingSize / billingThreadNum * i,
							billingSize / billingThreadNum * (i + 1));
				} else {
			...
			if ("0".equals(result.get("moc_call_count").trim()) && "0".equals(result.get("moc_duration").trim())
					&& "0".equals(result.get("billing_call_count").trim())
					&& "0".equals(result.get("billing_duration").trim())
					&& "0".equals(result.get("moc_call_count_without_billing").trim())
					&& "0".equals(result.get("moc_duration_without_billing").trim())
					&& "0".equals(result.get("billing_call_count_without_moc").trim())
					&& "0".equals(result.get("billing_duration_without_moc").trim())) {
				// System.out.println("all count are 0, so no
				// inserting/updating");
	...
	public boolean summaryRedo(String args[], String operator, Report report) throws SQLException, ParseException {
		...
		while (c.compareTo(base) > 0) {
			...
			result.put("transition_status", Constants.coreCfg.getTransitionStatus() != null
					? Constants.coreCfg.getTransitionStatus().getUpdatingFilename() : "1");

			// step7: insert count values.
			if ("0".equals(result.get("moc_call_count").trim()) && "0".equals(result.get("moc_duration").trim())
					&& "0".equals(result.get("in_call_count").trim()) && "0".equals(result.get("in_duration").trim())
					&& "0".equals(result.get("moc_call_count_without_in").trim())
					&& "0".equals(result.get("moc_duration_without_in").trim())
					&& "0".equals(result.get("in_call_count_without_moc").trim())
					&& "0".equals(result.get("in_duration_without_moc").trim())) {
				logger.info("all count are 0, so no inserting/updating");
