...
import com.mobius.ra.core.dao.IndRoamingSmyDao4Maldives;
 ...
 * @version v 1.0
 */
public class IndRoamingSmySvc4Maldives implements ServiceInterface {
	...
	private String getImsiPrefixTapCode(Map<String, String> tapCodeMap, long simsi) {
		...
		if (tapCodeMap != null) {
			imsiPrefixTapCode = tapCodeMap.get((simsi + "").substring(0, 6));
			if (imsiPrefixTapCode == null) {
				imsiPrefixTapCode = tapCodeMap.get((simsi + "").substring(0, 5));
			}
		}
		if (imsiPrefixTapCode == null) {
			logger.info("tapCode is not found in tap_code list, plz add this. imsi_prefix=" + (simsi + "").substring(0, 5) + ", imsi=" + simsi);
			imsiPrefixTapCode = (simsi + "").substring(0, 5) + ":Unkown:Unkown";
		}
	...
	public boolean summary(String args[], String operator, Report report) throws SQLException, ParseException {
		boolean flag = false;
		IndRoamingSmyDao4Maldives dao = new IndRoamingSmyDao4Maldives(report);
		...
		while (c.compareTo(base) > 0) {
			...
			// for data
			Map<String, Long> callCountMap4Data = new HashMap<String, Long>();
			Map<String, Long> volumeDLCountMap4Data = new HashMap<String, Long>();
			Map<String, Long> volumeUPCountMap4Data = new HashMap<String, Long>();

			long minCallId4In = 0;
			long maxCallId4In = 0;
			long minCallId4Out = 0;
			long maxCallId4Out = 0;
			long minCallId4Data = 0;
			long maxCallId4Data = 0;
			List<RaDetail> detailList = new ArrayList<RaDetail>();
			...
			for (CallsNoDup call : mscList) {
				...
				// data
				if ("12".equals(call.getCallType())) {
					minCallId4Data = call.getCallId();// give initial value
					// instead of 0.
					if (minCallId4Data > call.getCallId())
						minCallId4Data = call.getCallId();
					if (maxCallId4Data < call.getCallId())
						maxCallId4Data = call.getCallId();
					if ("0".equals(imsiPrefixTapCode)) {
						...
						long callCountData = callCountMap4Data.get(imsiPrefixTapCode) == null ? 0 : callCountMap4Data.get(imsiPrefixTapCode)
								.longValue();
						callCountMap4Data.put(imsiPrefixTapCode, callCountData + 1);
						// accumulate volume
						long volumeDLCount = volumeDLCountMap4Data.get(imsiPrefixTapCode) == null ? 0 : volumeDLCountMap4Data.get(imsiPrefixTapCode)
								.longValue();
						volumeDLCountMap4Data.put(imsiPrefixTapCode, volumeDLCount + call.getVolumeDownload());
						long volumeUPCount = volumeUPCountMap4Data.get(imsiPrefixTapCode) == null ? 0 : volumeUPCountMap4Data.get(imsiPrefixTapCode)
								.longValue();
						volumeUPCountMap4Data.put(imsiPrefixTapCode, volumeUPCount + call.getVolumeUpload());
						// put inbound roaming incoming into ra_detail.
						// @20141023, add tap code for ra_detail, only for 401
						// report.
						RaDetail rd = this.insertRaDetail4Msc(call, "17", "401");
			...
			List<IndRoamingSmy4Maldives> sumList = new ArrayList<IndRoamingSmy4Maldives>();

			// step 3.2.0: data for tapcode.
			Set<String> keys4Data = callCountMap4Data.keySet();// imsi_prefix
																// and
																// tap_code is
																// key.
			for (String imsiPrefixTapCode : keys4Data) {
				IndRoamingSmy4Maldives sum = new IndRoamingSmy4Maldives();
				sum.setTrafficDirection("");

				if (callCountMap4Data.containsKey(imsiPrefixTapCode))
					sum.setCallCountData(callCountMap4Data.get(imsiPrefixTapCode));

				if (volumeDLCountMap4Data.containsKey(imsiPrefixTapCode))
					sum.setDownloadVolumeCount(volumeDLCountMap4Data.get(imsiPrefixTapCode));

				if (volumeUPCountMap4Data.containsKey(imsiPrefixTapCode))
					sum.setUploadVolumeCount(volumeUPCountMap4Data.get(imsiPrefixTapCode));
				...
				sum.setCountryName(imsiPrefixTapCode.split(":")[2]);
				sum.setMinCallId(minCallId4Data);
				sum.setMaxCallId(maxCallId4Data);
